...
**背景：** 而家全部進度存喺單一 `localStorage` 鍵 `kaka-learn-v1`。朋友用**自己部機**開同一個 GitHub Pages 網址唔會撈亂；但**同一部 iPad／Mac 幾個小朋友輪流玩**會共用星星、PIN、設定。

**產品決定（暫緩）：** 優先改善基本認字／玩法功能；呢個多檔案方案留待之後再做。保持無登入、無雲端、無追蹤。

**建議方案（之後實作時跟）：**

1. Storage 升級 `v1` → `v2`：多 `profiles[]` + `activeProfileId`；每位小朋友獨立星星、字詞開關、靜音、（之後）弱項統計。
2. 主頁前／主頁加「邊個嚟學？」大頭像揀檔。
3. 家長區（PIN 後）可新增／改名／刪除檔案、重設該檔星星。
4. 舊 `kaka-learn-v1` 自動遷移成第一個檔（例如「卡卡」），唔掉進度。
5. 頂欄顯示「而家：某某」；文件說明「資料只存喺呢部機」。

**刻意唔做（除非產品改口）：** 登入／Google 帳號、雲端同步、網址 `?user=` 參數。

### 部首歸類學習（「邊個部首」）

**目標：** 將相同部首嘅字擺埋一齊，等小朋友慢慢掌握學習規律／見字識類。

**例子：**
- 犬字部動物：狗、貓、狼、狐狸、猴、獅（視乎收字）
- 月（肉月）旁身體：肚、腳、手（膊）、頭、心…（按字表）
- 草花頭、口字旁、木字旁等

**方向（之後設計）：**
- 每個字詞資料可加 `radical`（部首）欄位；自動分組出「部首小家族」
- 學習頁／小遊戲：「邊啲字係犬字部？」「月旁多數係身體」
- 唔同考認字獨立——係**歸類＋規律**練習

**產品決定（暫緩）：** 先擴充主題同基本玩法；呢個屬中階功能，留待之後同家長一齊定收字範圍。

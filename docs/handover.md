...
- 《信》`flow: 'gentle'`；其餘正式本（《分果果》《誰在叫》《快跑呀》）暫維持先學再測＋干擾。
- **踩咗** `data/red-series/sentence-game-data.mjs`、`js/book-scene-demo.js`、`book-scene-demo.html`、`scripts/test-book-scene-demo.mjs`、`scripts/smoke-book-scene-demo.py`、`docs/handover.md`
...
- 加法及減法所有物件加入獨立量詞資料：士多啤梨「粒」、雞仔／小白兔「隻」、金幣「枚」、星星「顆」
- 題目、錯誤提示、完成語音及無障礙描述全部由同一份量詞資料生成，避免畫面同聲音不一致
- **踩咗** `js/additionData.js`、`js/additionGame.js`、`js/subtractionData.js`、`js/subtractionGame.js`、`docs/handover.md`

### 2026-09-08 · Codex（減法回答確認掣＋桌面版面修正）

- 月球減法改為「先操作、再回答」：撳／拖物件只會移動到拿走區，必須撳「回答」先判斷，數量不足會提示而不會跳題
- 減法桌面版改用自適應兩欄：白板佔主欄、拿走區固定副欄；總寬由容器計算，避免白板同拿走區重疊；手機版自動上下排列
- **踩咗** `index.html`、`js/subtractionGame.js`、`css/additionGame.css`、`docs/handover.md`
...
- `pth-demo.html` 的 `css/pth-demo.css` 及 `js/pth-demo.js` 已加共用版本 query `?v=20260914-pth-word-audio`。
- 正確預覽網址：`http://127.0.0.1:5174/pth-demo.html?v=20260914-pth-word-audio`；如仍見舊版，關閉舊 tab 後重新開啟此網址。

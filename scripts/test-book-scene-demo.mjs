...
const pageManifest = JSON.parse(fs.readFileSync(path.join(root, 'data/red-series/story-page-manifest.json'), 'utf8'));
...
assert.match(source, /assets\/book-scenes\/red-series-pages-hq\/\$\{title\}-p4\.webp/);
assert.match(source, /真實故事頁・頁面配對待核實/);
assert.equal(pageManifest.pages.length, 10);
assert.ok(pageManifest.pages.every((book) => book.items.length === 1 && book.items[0].page === 4));
assert.equal(fs.readdirSync(path.join(root, 'assets/book-scenes/red-series-pages-hq')).length, 10);
assert.equal(fs.readdirSync(path.join(root, 'assets/book-scenes/red-series-pages-hq')).filter((name) => !/-p4\.webp$/.test(name)).length, 0);

...
assert.ok(pageManifest.pages.every((book) => book.items.length >= 1));
const pageAssets = fs.readdirSync(path.join(root, 'assets/book-scenes/red-series-pages-hq'));
assert.equal(pageAssets.length, 11);
assert.equal(pageAssets.filter((name) => !/-p4\.webp$/.test(name)).length, 1);

...
assert.match(source, /assets\/book-scenes\/red-series-pages\/\$\{title\}-p/);
...
assert.equal(fs.readdirSync(path.join(root, 'assets/book-scenes/red-series-pages')).length, 10);
assert.equal(fs.readdirSync(path.join(root, 'assets/book-scenes/red-series-pages')).filter((name) => /-p[56]\.jpg$/.test(name)).length, 0);

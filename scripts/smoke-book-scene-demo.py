...
def run(url: str, out: Path | None) -> None:
    ...
    with sync_playwright() as playwright:
        ...
        for name, (width, height) in VIEWPORTS.items():
            ...
            assert page.locator('#scene-start-quiz').is_hidden(), f"{name}: gentle 不應有開始小測"
            assert page.locator('#scene-submit').is_hidden(), f"{name}: gentle 不應有檢查掣"
            assert page.locator('#scene-study').is_hidden(), f"{name}: gentle 不應預先顯示答案句"
            assert page.locator('.answer-slot').count() == 4, f"{name}: 整個跨頁應有四個詞組槽"
            assert page.locator('.word-card').count() == 4, f"{name}: gentle 字池只有正確詞組"
            assert page.locator('.word-card.is-distractor').count() == 0, f"{name}: gentle 不可有干擾字"
            ...
            page.locator('.word-card[data-word="媽媽"]').click()
            page.locator('.word-card[data-word="一個蘋果"]').click()
            page.wait_for_function("document.querySelector('#scene-feedback').textContent.includes('砌好喇')")
            ...
            assert page.locator('.word-card').count() == 4, f"{name}: 再玩後仍應即時顯示字池"
            ...
            assert page.locator('#scene-start-quiz').is_hidden(), f"{name}: 《誰在叫》gentle 不應有開始小測"
            assert page.locator('#scene-study').is_hidden(), f"{name}: 《誰在叫》gentle 不應預先顯示答案句"
            ...
            assert page.locator('.word-card').count() == 10, f"{name}: 《誰在叫》gentle 只有十個答案詞組"
            assert page.locator('.word-card.is-distractor').count() == 0, f"{name}: 《誰在叫》不可有干擾字"
            ...
            page.wait_for_function("document.querySelector('#scene-feedback').textContent.includes('砌好喇')")
            assert '砌好喇' in page.locator('#scene-feedback').inner_text(), f"{name}: 正確句子未被接受；feedback={page.locator('#scene-feedback').inner_text()}"
            page.wait_for_function("document.querySelector('#star-count').textContent.startsWith('1/10')")
